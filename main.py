...
def parse_args():
    ...
    parser.add_argument("--soft_prompt_type", type=str, default="add", choices=["front", "add"], help="Type of soft prompt to use")
    ...
    parser.add_argument("--output_dir", type=str, default="results_proposed", help="Directory to save the model")
    parser.add_argument("--num_train_data", type=int, default=10000, help="Number of training data points")
    parser.add_argument("--num_eval_data", type=int, default=5000, help="Number of evaluation data points")
...
def main():
    ...
    if args.use_soft_prompt:
        if args.soft_prompt_type == "front":
            args.run_name += f"_soft_prompt_front"
        elif args.soft_prompt_type == "add":
            args.run_name += f"_soft_prompt_add"
 
    if args.knowledge_deleting or args.current_prob_based_loss:
        args.run_name += f"_proposed_"
        if args.current_prob_based_loss:
            args.run_name += f"cpbl"
        if args.knowledge_deleting:
            args.run_name += f"-kd"
            if args.knowledge_deleting_loss_type == "ppo":
                args.run_name += f"-ppo"
                
                
            if args.knowledge_deleting_finish_strategy == "step":
                args.run_name += f"-s{args.knowledge_deleting_steps}"
            elif args.knowledge_deleting_finish_strategy == "max_value":
                args.run_name += f"-mv{args.knowledge_deleting_max_value}"
    ...
    print(f"load pretrained model from {args.checkpoint_path}")
    ...
    if args.knowledge_deleting:
        model = train(args, model, tokenizer, data, data, args.original_target_file, mode="knowledge_deleting")
    ...
    if args.add_pretrained_data:
        ...
        if args.use_soft_prompt:
            train_with_soft_prompt(args, model, tokenizer, repeated_data, data, original_data, mode="train")
        else:
            train(args, model, tokenizer, repeated_data, data, original_data, mode="train")
    else:
        if args.use_soft_prompt:
