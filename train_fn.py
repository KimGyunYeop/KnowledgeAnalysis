...
def train_with_soft_prompt(args, model, tokenizer, train_data, test_data, original_data=None, mode="train"):
    ...
    for i in tqdm(range(total_steps)):
        ...
        soft_prompt_optimizer = torch.optim.Adam([soft_prompt], lr=1e-3)
        ...
        for j in range(10000):
            ...
            if soft_prompt_loss.mean() < 0.5:
                use_soft_prompt = j > 0

                print(f"Early stopping at soft prompt create step {j}")
                break
            loss = soft_prompt_loss.mean()
            loss.backward()
